                ...
                <v-col class="v-col-lg-9">
                    <div class="zo-info d-lg-flex d-sm-none">
                        <div class="zo-icon elevation-4">
                            <i class="mdi mdi-format-list-group-plus"></i>
                        </div>
                        <div class="zo-name">
...
const handleSave = async (category) => {
    const url = isEditing.value
        ? route('admin.books.categories.update', {category: category.id})
        : route('admin.books.categories.store')
    ...
    try {
        router[method](url, category, {
            ...
            onSuccess: () => {
                ...
                if (!isEditing.value) {
                    router.visit(route('admin.books.categories.index', {page: 1}), {
                        preserveScroll: true,
                        preserveState: true,
                        replace: true,
                        only: ['categories'],
...
const changePage = (page) => {
    router.visit(route('admin.books.categories.index', {page}), {
        preserveState: true,
        preserveScroll: true,
        replace: true
    })
