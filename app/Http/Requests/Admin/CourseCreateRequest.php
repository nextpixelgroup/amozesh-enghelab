...
class CourseCreateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'seasons.*.lessons.*.quiz.questions.*.type'        => 'required|in:true_false,multiple_choice,descriptive',
            'seasons.*.lessons.*.quiz.questions.*.explanation' => 'nullable|string',
            'seasons.*.lessons.*.quiz.questions.*.options'     => 'required_if:seasons.*.lessons.*.quiz.questions.*.type,true_false,multiple_choice|array|min:2',
