import { Head, Link, usePage, useForm as useForm$1, router, createInertiaApp } from "@inertiajs/vue3";
import { defineComponent as defineComponent$1, useSSRContext, resolveComponent, shallowRef, isVNode, Comment, Fragment, camelize, reactive, watchEffect, toRef, capitalize, unref, warn, getCurrentInstance as getCurrentInstance$1, inject as inject$1, computed, provide, ref, h, onBeforeUnmount, watch, readonly, onMounted, useId, onDeactivated, onActivated, onScopeDispose, effectScope, toRaw, getCurrentScope, createElementVNode, normalizeStyle, normalizeClass, createVNode, withCtx, createTextVNode, renderSlot as renderSlot$1, isRef, toValue, onUpdated, mergeProps, toRefs, Text, Transition, resolveDynamicComponent, nextTick, withDirectives, toDisplayString, markRaw, TransitionGroup, vShow, onBeforeMount, cloneVNode, withModifiers, Teleport, normalizeProps, guardReactiveProps, onUnmounted, onBeforeUpdate, vModelText, render, createSSRApp } from "vue";
...
function focusableChildren(el) {
  ...
  return [...el.querySelectorAll(targets)].filter((x) => !x.closest("[inert]")).filter((x) => !!x.offsetParent || x.getClientRects().length > 0).filter((x) => !x.parentElement?.closest("details:not([open])") || x.tagName === "SUMMARY" && x.parentElement?.tagName === "DETAILS");
}
...
const _sfc_main$9 = defineComponent$1({
  components: { AdminLayout: _sfc_main$i, Head }
});
...
function formHandler(initialValues = null, method = "post") {
  ...
  usePage();
  const form = useForm$1(initialValues);
...
function mounted$4(el, binding) {
  ...
  const observer = new IntersectionObserver(function() {
    ...
    if (isIntersecting && modifiers.once) unmounted$4(el, binding);
  ...
  _push(ssrRenderComponent(_component_AdminLayout, _attrs, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        ...
        _push2(ssrRenderComponent(VForm, { onSubmit: () => {
        } }, {
          default: withCtx((_2, _push3, _parent3, _scopeId2) => {
            if (_push3) {
...
const VMenu = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    async function onFocusIn(e) {
      ...
      await nextTick();
      if (isActive.value && before !== after && overlay.value?.contentEl && // We're the menu without open submenus or overlays
